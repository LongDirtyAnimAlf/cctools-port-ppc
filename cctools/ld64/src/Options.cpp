...
Options::FileInfo Options::findFile(const char* apath)
{
	FileInfo result;
	struct stat statBuffer;
	char *p,*s;
    ...
    *p = 0;
	...
	while ( path.find ("\r") != std::string::npos )
	{
		path.erase ( path.find ("\r"), 1 );
	}
    */
