...
{
	fStartLoadAndResolveTime = xarch_absolute_time();
		...
		switch ( it->type ) {
			...
			case N_SO:
				if ( (it->string != NULL) && (strlen(it->string) > 0) ) {
					...
					soRanges.push_back(std::make_pair(atomWithLowestOrdinal, atomWithHighestOrdinal));
				}
				// fall through
			default:
				if ( curRangeIndex != -1 ) {
...
void Linker::collectDebugInfo()
{
	...
	fStartDebugTime = xarch_absolute_time();
